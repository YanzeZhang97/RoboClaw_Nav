...
def test_lifecycle_reset_world_uses_bringup_ros_domain_id(tmp_path) -> None:
    script = tmp_path / "robotics" / "scripts" / "run_sim.sh"
    script.parent.mkdir(parents=True)
    script.write_text("#!/usr/bin/env bash\n", encoding="utf-8")
    seen = {}
    ...
    lifecycle = SimulationLifecycle(
        repo_root=tmp_path,
        process_factory=lambda argv, cwd, env: _FakeProcess(),
        shell_runner=shell_runner,
    )
    lifecycle.bringup(mode="gazebo", ros_domain_id=7)
    result = lifecycle.reset_world()

    assert result["ok"] is True
    assert lifecycle.status()["ros_domain_id"] == 7
